<template>
	<form @submit='searchSubmit'>
		<div class="search-wrapper container row">
			<!-- 定位 -->
	      <router-link class="search-wrapper__url white" :to="dingUrl" v-if='hasDing'>东莞</router-link>
	      <div class='search-wrapper__box flex-1 white rel container row'>
	      	...
	      	<!-- 遮罩层 -->
	      	<router-link class="search-wrapper__mask abs" :to="jumpUrl" v-if='isJump'></router-link>
	      </div>
	      <!-- 默认放置搜索按钮 -->
	      <slot>
	    ...
	    <div class="search-wrapper--fix"></div>
	</form>
</template>
